...
class ProductDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query->with('category')))
            ...
            ->addColumn('category_id', function ($row) {
                return e(optional($row->category)->category_name); // Access 'category_name' instead of 'name'
            })
            ->addColumn('product_image', function ($row) {
                if ($row->product_image) {
                    $url = asset($row->product_image); // Safe even if it's relative
                    return '<img src="' . $url . '" alt="Category Image" width="60" height="60">';
                } else {
                    return '<span class="text-muted">No Image</span>';
                } // Use e() for escaping output
            ...
            ->addColumn('product_desc', function ($row) {
                return e($row->product_desc); // Use e() for escaping output
            });
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('product_image')->title('Image'),
            Column::make('product_name')->title('Product Name'),
            Column::make('product_mrp_price')->title('Price Name'),
