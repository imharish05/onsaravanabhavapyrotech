...
// AUTO-GENERATED TEST FOR MODULE: SubCategory
echo "\n============================================\n";
echo "🚀 Running Advanced QA Tests for SubCategory\n";
echo "============================================\n";
...
// Testing GET /subcategory/view
$ch = curl_init($baseUrl . "/subcategory/view");
...
if($httpcode >= 200 && $httpcode < 400) {
    echo "✅ [PASS] GET /subcategory/view -> index() successful (HTTP $httpcode)\n";
} else if ($httpcode === 419) {
    echo "❌ [FAIL] GET /subcategory/view -> index() CSRF check failed (HTTP $httpcode)\n";
} else if ($httpcode === 401 || $httpcode === 403) {
    echo "🛡️ [AUTH] GET /subcategory/view -> index() Restricted Access (HTTP $httpcode)\n";
} else {
    echo "⚠️ [INFO] GET /subcategory/view -> index() threw error (HTTP $httpcode) (Check logs if 500)\n";
}
curl_close($ch);

// Testing POST /subcategory/add
$ch = curl_init($baseUrl . "/subcategory/add");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_COOKIEFILE, $cookieFile);
...
$httpcode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
if($httpcode >= 200 && $httpcode < 400) {
    echo "✅ [PASS] POST /subcategory/add -> addsubcategory() successful (HTTP $httpcode)\n";
} else if ($httpcode === 419) {
    echo "❌ [FAIL] POST /subcategory/add -> addsubcategory() CSRF check failed (HTTP $httpcode)\n";
} else if ($httpcode === 401 || $httpcode === 403) {
    echo "🛡️ [AUTH] POST /subcategory/add -> addsubcategory() Restricted Access (HTTP $httpcode)\n";
} else {
    echo "⚠️ [INFO] POST /subcategory/add -> addsubcategory() threw error (HTTP $httpcode) (Check logs if 500)\n";
}
curl_close($ch);
...
$profiles = [50, 100, 200];
echo "⏳ Load Testing /subcategory/view...\n";
