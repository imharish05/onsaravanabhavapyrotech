...
// AUTO-GENERATED TEST FOR MODULE: HomeSetting
echo "\n============================================\n";
echo "🚀 Running Advanced QA Tests for HomeSetting\n";
echo "============================================\n";
...
// Testing GET /home-settings
$ch = curl_init($baseUrl . "/home-settings");
...
if($httpcode >= 200 && $httpcode < 400) {
    echo "✅ [PASS] GET /home-settings -> index() successful (HTTP $httpcode)\n";
} else if ($httpcode === 419) {
    echo "❌ [FAIL] GET /home-settings -> index() CSRF check failed (HTTP $httpcode)\n";
} else if ($httpcode === 401 || $httpcode === 403) {
    echo "🛡️ [AUTH] GET /home-settings -> index() Restricted Access (HTTP $httpcode)\n";
} else {
    echo "⚠️ [INFO] GET /home-settings -> index() threw error (HTTP $httpcode) (Check logs if 500)\n";
}
curl_close($ch);

// Testing POST /home-settings
$ch = curl_init($baseUrl . "/home-settings");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_COOKIEFILE, $cookieFile);
...
$httpcode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
if($httpcode >= 200 && $httpcode < 400) {
    echo "✅ [PASS] POST /home-settings -> update() successful (HTTP $httpcode)\n";
} else if ($httpcode === 419) {
    echo "❌ [FAIL] POST /home-settings -> update() CSRF check failed (HTTP $httpcode)\n";
} else if ($httpcode === 401 || $httpcode === 403) {
    echo "🛡️ [AUTH] POST /home-settings -> update() Restricted Access (HTTP $httpcode)\n";
} else {
    echo "⚠️ [INFO] POST /home-settings -> update() threw error (HTTP $httpcode) (Check logs if 500)\n";
}
curl_close($ch);
...
$profiles = [50, 100, 200];
echo "⏳ Load Testing /home-settings...\n";
