...
// AUTO-GENERATED TEST FOR MODULE: Discount
echo "\n============================================\n";
echo "🚀 Running Advanced QA Tests for Discount\n";
echo "============================================\n";
...
// Testing POST /discount/add
$ch = curl_init($baseUrl . "/discount/add");
...
if($httpcode >= 200 && $httpcode < 400) {
    echo "✅ [PASS] POST /discount/add -> adddiscount() successful (HTTP $httpcode)\n";
} else if ($httpcode === 419) {
    echo "❌ [FAIL] POST /discount/add -> adddiscount() CSRF check failed (HTTP $httpcode)\n";
} else if ($httpcode === 401 || $httpcode === 403) {
    echo "🛡️ [AUTH] POST /discount/add -> adddiscount() Restricted Access (HTTP $httpcode)\n";
} else {
    echo "⚠️ [INFO] POST /discount/add -> adddiscount() threw error (HTTP $httpcode) (Check logs if 500)\n";
}
curl_close($ch);
...
$profiles = [50, 100, 200];
echo "⏳ Load Testing /discount/add...\n";
