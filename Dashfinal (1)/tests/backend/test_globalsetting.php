...
// AUTO-GENERATED TEST FOR MODULE: GlobalSetting
echo "\n============================================\n";
echo "🚀 Running Advanced QA Tests for GlobalSetting\n";
echo "============================================\n";
...
// Testing GET /admin/global-settings
$ch = curl_init($baseUrl . "/admin/global-settings");
...
if($httpcode >= 200 && $httpcode < 400) {
    echo "✅ [PASS] GET /admin/global-settings -> index() successful (HTTP $httpcode)\n";
} else if ($httpcode === 419) {
    echo "❌ [FAIL] GET /admin/global-settings -> index() CSRF check failed (HTTP $httpcode)\n";
} else if ($httpcode === 401 || $httpcode === 403) {
    echo "🛡️ [AUTH] GET /admin/global-settings -> index() Restricted Access (HTTP $httpcode)\n";
} else {
    echo "⚠️ [INFO] GET /admin/global-settings -> index() threw error (HTTP $httpcode) (Check logs if 500)\n";
}
curl_close($ch);

// Testing PUT /admin/global-settings/update
$ch = curl_init($baseUrl . "/admin/global-settings/update");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_COOKIEFILE, $cookieFile);
...
$httpcode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
if($httpcode >= 200 && $httpcode < 400) {
    echo "✅ [PASS] PUT /admin/global-settings/update -> update() successful (HTTP $httpcode)\n";
} else if ($httpcode === 419) {
    echo "❌ [FAIL] PUT /admin/global-settings/update -> update() CSRF check failed (HTTP $httpcode)\n";
} else if ($httpcode === 401 || $httpcode === 403) {
    echo "🛡️ [AUTH] PUT /admin/global-settings/update -> update() Restricted Access (HTTP $httpcode)\n";
} else {
    echo "⚠️ [INFO] PUT /admin/global-settings/update -> update() threw error (HTTP $httpcode) (Check logs if 500)\n";
}
curl_close($ch);
...
$profiles = [50, 100, 200];
echo "⏳ Load Testing /admin/global-settings...\n";
