...
// AUTO-GENERATED TEST FOR MODULE: OnOff
echo "\n============================================\n";
echo "🚀 Running Advanced QA Tests for OnOff\n";
echo "============================================\n";
...
// Testing GET /onoff/view
$ch = curl_init($baseUrl . "/onoff/view");
...
if($httpcode >= 200 && $httpcode < 400) {
    echo "✅ [PASS] GET /onoff/view -> index() successful (HTTP $httpcode)\n";
} else if ($httpcode === 419) {
    echo "❌ [FAIL] GET /onoff/view -> index() CSRF check failed (HTTP $httpcode)\n";
} else if ($httpcode === 401 || $httpcode === 403) {
    echo "🛡️ [AUTH] GET /onoff/view -> index() Restricted Access (HTTP $httpcode)\n";
} else {
    echo "⚠️ [INFO] GET /onoff/view -> index() threw error (HTTP $httpcode) (Check logs if 500)\n";
}
curl_close($ch);

// Testing POST /pageoff/update
$ch = curl_init($baseUrl . "/pageoff/update");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_COOKIEFILE, $cookieFile);
...
$httpcode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
if($httpcode >= 200 && $httpcode < 400) {
    echo "✅ [PASS] POST /pageoff/update -> updatepage() successful (HTTP $httpcode)\n";
} else if ($httpcode === 419) {
    echo "❌ [FAIL] POST /pageoff/update -> updatepage() CSRF check failed (HTTP $httpcode)\n";
} else if ($httpcode === 401 || $httpcode === 403) {
    echo "🛡️ [AUTH] POST /pageoff/update -> updatepage() Restricted Access (HTTP $httpcode)\n";
} else {
    echo "⚠️ [INFO] POST /pageoff/update -> updatepage() threw error (HTTP $httpcode) (Check logs if 500)\n";
}
curl_close($ch);
